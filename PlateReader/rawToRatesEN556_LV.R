...
master <- "EN556_plate_ratesLV.csv"
...
for (file in RawNameListLV) {
    ...
    PartialName <- sub("stn([1-9]+)-d([1-5])-([A-Za-z]+)-([a-z]+[0-9])-subt([0-9])-t([0-9]).raw","stn\\1-d\\2-\\3",file)
    TreatName <- sub("stn([1-9]+)-d([1-5])-([A-Za-z]+)-([a-z]+[0-9])-subt([0-9])-t([0-9]).raw","stn\\1-d\\2-\\3-\\4",file)
    SubTName <- sub("stn([1-9]+)-d([1-5])-([A-Za-z]+)-([a-z]+[0-9])-subt([0-9])-t([0-9]).raw","stn\\1-d\\2-\\3-\\4-subt\\5",file)
    FullName <- sub("stn([1-9]+)-d([1-5])-([A-Za-z]+)-([a-z]+[0-9])-subt([0-9])-t([0-9]).raw","stn\\1-d\\2-\\3-\\4-subt\\5-t\\6",file)
    ...
    ExptList[[PartialName]][[TreatName]][[SubTName]][[FullName]] = read.table(paste(RawDir,file,sep="/"), skip=5, na.strings=c("NA","LOW"),row.names=c("x1","x2","x3","rep1_rate","rep2_rate","rep3_rate"), col.names=c("","a","b","L","p1","p2","p3","p4", rep("empty",5)), colClasses = c("NULL",rep("integer", 7), rep("NULL", 5)), nrows=6)
}
    ...
    for (treat in 1:length(ExptList[[inc]])) {
        for (subt in 1:length(ExptList[[inc]][[treat]])) {
            ...
            for (substrate in names(fl.list)) {
                ...
                plateMasterLV <- rbind(plateMasterLV, newrow)
...
write.csv(plateMasterLV,file=master,row.names=TRUE)
