...
master <- "Plate_RatesLV_EN556.csv"
...
for (file in RawNameListLV) {
    ...
    ExptList[[PartialName]][[TreatName]][[SubTName]][[FullName]] = read.table(paste(RawDir,file,sep="/"), skip=5, na.strings=c("NA","LOW"),row.names=c("x1","x2","x3","live1","live2","live3"), col.names=c("","a","b","L","p1","p2","p3","p4", rep("empty",5)), colClasses = c("NULL",rep("integer", 7), rep("NULL", 5)), nrows=6)
}
