#FLARatesEN556_GF for hydrolysis rate calculations from gravity filtration incubations

#basically the same as bulk water calculations, except need to correct for amount water filtered 
#and fraction of filter that went into each incubation

#EN556 labeling scheme: stn#-d#-bulk/GF/LV-substrate-rep-t#
#e.g. stn7-d0-bulk-lam-x-t3 or stn4-d5-GF-chon-1-t0
...
#incubation volume for GF incs in EN556 is 15mL=0.015L
IncVolume <- 0.015
...
outputMaster <- "EN556_FLA_ratesGF.csv"
...
#only want to look at GF incs for this script
CsvNameListGF <- CsvNameList[grep("stn([0-9]+)-d([0-5])-GF-([a-z]+)-([0-9a-z])-t([0-9]).csv",CsvNameList)]
...
for (i in 1:length(CsvNameListGF)) {
    CsvName <- CsvNameListGF[i]
...
for (j in 1:length(RatesMasterList)) {
    ...
    for (aa in substrates) {
        if (grepl(paste("*",aa,sep=""),names(RatesMasterList[j]))==TRUE) {
            ...
            nmolMonInVial <- MonEquivSubConc[,aa]*1000*IncVolume #nmol of monomer equivalents in the incubation
    ...
    #calculate rates from hydrolysis events - retrieve elapsed time from spreadsheet, divide hydrolysisEvents/timeElapsed/filtered volume in incubation
